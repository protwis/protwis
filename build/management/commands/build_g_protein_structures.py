...
class Command(BaseBuild):
	...
	def handle(self, *args, **options):
		...
		for sc in scs:
			...
			for s in seqadv:
				line_search = re.search('SEQADV\s{1}[A-Z\s\d]{4}\s{1}([A-Z]{3})\s{1}([A-Z]{1})\s+(\d+)[\s\S\d]{17}([A-Z]{3})\s+(\d+)\s\S+',s)
				if line_search!=None:
					if line_search.group(2)==sc.alpha:
						if line_search.group(3)==line_search.group(5):
							mutations[int(line_search.group(3))] = [line_search.group(1), line_search.group(4)]
						else:
							shifted_mutations[int(line_search.group(3))] = [line_search.group(1), line_search.group(4), int(line_search.group(5))]
