...
class Command(BaseBuild):
    ...
    def order_assays(assays):
        try:
            sorted_assay = sorted(assays, key=lambda k: k['transduction_coef'], reverse=True)
        except:
            sorted_assay = sorted(assays, key=lambda k: k['relative_transduction_coef']
                                  if k['relative_transduction_coef'] else float(-1000), reverse=True)
    ...
    def get_rid_of_gprot(self, assay, families, proteins):
        if assay['family'] == 'Gq/11 or Gi/o':
            ...
            if compare_val_gq is None and compare_val_gio is not None:
                try:
                    if assay['delta_emax_ec50'] > compare_val_gio['delta_emax_ec50']:
                ...
                try:
                    if assay['delta_emax_ec50'] > compare_val_gq['delta_emax_ec50']:
                        families[:] = [d for d in families if d.get(
                            'family') != compare_val_gio['family']]
                        assay['family'] = 'Gq/11'
                    else:
                ...
                try:
                    if assay['delta_emax_ec50'] > compare_val_gio['delta_emax_ec50'] > compare_val_gq['delta_emax_ec50']:
                        families[:] = [d for d in families if d.get(
                            'family') != compare_val_gq['family']]
                        assay['family'] = 'Gq/11'
                    elif assay['delta_emax_ec50'] > compare_val_gq['delta_emax_ec50'] > compare_val_gio['delta_emax_ec50']:
                        families[:] = [d for d in families if d.get(
                            'family') != compare_val_gio['family']]
                        assay['family'] = 'Gi/o'
                    elif compare_val_gq['delta_emax_ec50'] > assay['delta_emax_ec50'] > compare_val_gio['delta_emax_ec50']:
                        families[:] = [d for d in families if d.get(
                            'family') != compare_val_gio['family']]
                        assay['family'] = 'Gi/o'
                    elif compare_val_gio['delta_emax_ec50'] > assay['delta_emax_ec50'] > compare_val_gq['delta_emax_ec50']:
                        families[:] = [d for d in families if d.get(
                            'family') != compare_val_gq['family']]
                        assay['family'] = 'Gq/11'
                except:
    ...
    def calc_bias_factor(biasdata):
        ...
        for i in biasdata:
            if i['order_no'] == 0:
                if i['relative_transduction_coef'] is None:
                    i['calculated_relative_tau'] = Command.calculate_relative_transduction_coef(i)
                most_potent = i
        for i in biasdata:
            if i['order_no'] != 0:
                try:
        ...
        if most_potent['calculated_relative_tau'] is not None:
            if i['transduction_coef']:
                try:
                    ...
                    pre_result = None
                    result = None
            elif i['relative_transduction_coef']:
                try:
                    if i['endogenous_assay']['relative_transduction_coef'] and float(i['endogenous_assay']['relative_transduction_coef']) == 0.0:
            ...
            try:
                if most_potent['endogenous_assay']['relative_transduction_coef'] and float(most_potent['endogenous_assay']['relative_transduction_coef']) == 0.0:
                    try:
                        pre_result = i['relative_transduction_coef']
                        result = most_potent['relative_transduction_coef'] - i['relative_transduction_coef']
                    except Exception:
    ...
    @staticmethod
    def lbf_calculate_bias_parts(i):
        result = None
        try:
            c = 0
            d = 0
            temp_reference = i['endogenous_assay']
    ...
    @staticmethod
    def caclulate_bias_factor_variables(a, b, c, d):
        '''
