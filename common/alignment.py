...
class Alignment:
    ...
    def load_segments(self, selected_segments):
        ...
        for s in selected_segments:
            ...
            # FIXME: selects segments from all proteinfamilies

            if self.numbering_schemes[0][0] == 'can':
                alt_segments = ProteinSegment.objects.filter(slug__startswith=selected_segment.slug, proteinfamily='Arrestin')
            elif self.numbering_schemes[0][0] == 'cgn':
                alt_segments = ProteinSegment.objects.filter(slug__startswith=selected_segment.slug, proteinfamily='Gprotein')
            else:
