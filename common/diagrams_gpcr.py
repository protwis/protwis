...
# Generated by common/diagrams_gpcr_constants_calc.py - re-run that script and paste its
# output here if the anchor choices need updating as more structures become available.
# Per TM: ordered cross-class generic-number candidates (primary first, then fallbacks)
# used to align each TM helix in the snake plot.
TM_ANCHOR_CANDIDATES = {
    1: ['1x44', '1x42', '1x43'],  # mean midplane_distance: 1x44: +0.25, 1x42: +1.59, 1x43: +1.90
    2: ['2x52', '2x53', '2x54'],  # mean midplane_distance: 2x52: -0.74, 2x53: +0.99, 2x54: +1.23
    3: ['3x35', '3x36', '3x37'],  # mean midplane_distance: 3x35: +0.06, 3x36: +0.37, 3x37: -0.91
    4: ['4x54', '4x53', '4x55'],  # mean midplane_distance: 4x54: +0.75, 4x53: -1.05, 4x55: +1.68
    5: ['5x46', '5x461', '5x47'],  # mean midplane_distance: 5x46: +0.00, 5x461: -0.24, 5x47: -0.75
    6: ['6x48', '6x49', '6x47'],  # mean midplane_distance: 6x48: +0.18, 6x49: +1.05, 6x47: -1.05
...
class DrawSnakePlot(Diagram):
    ...
    def __init__(self, residue_list, protein_class, protein_name, nobuttons = None, domain=None, residue_distance_lookup=None):
        ...
        if self.family.startswith('Class B2') and self.domain=='GAIN':
            ...
            for i in range(1,8):
                try:
                    helix_outputs[i] = self.drawSnakePlotHelix(i)
                    ...
                    helix_outputs[i] = ''
                    self.tm_alignment_status[i] = 'missing'
            ...
            for i in range(1,8):
                ...
                if shift:
                    helix_outputs[i] = "<g transform='translate(0,{:.2f})'>".format(shift) + helix_outputs[i] + "</g>"
                self.helixoutput += helix_outputs[i]
            ...
            if "H8" in self.segments: #if helix8
                ...
                if self.TBCoords[8]['top'][1] < self.low: self.low = self.TBCoords[8]['top'][1]
                if self.TBCoords[8]['bottom'][1] > self.high: self.high = self.TBCoords[8]['bottom'][1]
            ...
            if messages:
                ...
                warn_y = (min(tm_tops) if tm_tops else self.offsetY) - 25
                self.overlayoutput = ("<circle cx='{}' cy='{}' r='12' fill='#f0ad4e' stroke='black' "
                    "stroke-width='1' title='{}' original_title='{}'/>"
                    "<text x='{}' y='{}' text-anchor='middle' dominant-baseline='middle' font-family='helvetica' "
                    "font-size='16' font-weight='bold' fill='white' title='{}' original_title='{}'>!</text>").format(
                    warn_x, warn_y, warning, warning, warn_x, warn_y+1, warning, warning)
    ...
    def drawSnakePlotHelix(self, helix_num):
        ...
        indentY = 3
        row_residues = []
        anchor_candidates = TM_ANCHOR_CANDIDATES.get(helix_num, [])
        anchor_matches = {}
        residue_positions = []
        for i in range(0,res_num):
            ...
            if rs[i][2] in anchor_candidates:
                anchor_matches[rs[i][2]] = y

            distances = self.residue_distance_lookup.get(rs[i][0])
            if distances:
                midplane_value = distances['best_midplane'] if distances['best_midplane'] is not None else distances['avg_midplane']
                if midplane_value is not None:
                    residue_positions.append((y, midplane_value))
        ...
        for (y1, v1), (y2, v2) in zip(residue_positions, residue_positions[1:]):
            if (v1 >= 0) != (v2 >= 0) or v1 == 0 or v2 == 0:
                total = abs(v1) + abs(v2)
                t = abs(v1)/total if total else 0.5
