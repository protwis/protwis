/*global Raphael, XMLObjectifier, ActiveXObject, bam*/
...
Smits.Common = {
	...
	addRaphEventHandler(el, eventType, fn, paramsObj) {
		try {
	...
	createGradientEl(name, obj, coords) {
...
Smits.PhyloCanvas.Node.prototype = {
	...
	getMidbranchPosition(firstBranch) {
...
Smits.PhyloCanvas.Render.Style = {
	...
	getStyle(requestStyle, fallbackStyle) {
...
Smits.PhyloCanvas.Render.SVG.prototype = {
	...
	render() {
		...
		for (var i = 0; i < instructs.length; i++) {
			if (instructs[i].type === "line") {
				...
				if (instruct[i].attr("text-anchor") === "middle") {
	...
	draw(instruct) {
		...
		if (instruct.type === "line") {
			...
			if (instruct.attr["text-anchor"] === "middle") {
				obj.node.setAttribute("class", "label")
			}
			if (instruct.attr) {
	...
	var svg,
		...
		recursiveCalculateNodePositions = function(node, positionX) {
			...
			if (node.children.length > 0) {
				...
				console.log(Smits.PhyloCanvas.Render.Style.line);
				...
				svg.draw(
					new Smits.PhyloCanvas.Render.Path(
						array_merge(
							...
							"L", secant(positionX, minAngle, maxAngle, {
								noMove: true
							}),
	...
	return function(sSvg, dataObject, bufferRadius) {
		...
		if (dataObject.getValidate()) {
			sSvg.draw({
				type: "text",
				x: 0,
				y: sSvg.canvasSize[1] / 3,
				text: dataObject.getValidate()
			});
			return
		}
