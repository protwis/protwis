...
function updateTargetCount(){
  ...
  if (numTargets === 1){
    message += " ligand selected";
  } else {
    message += " ligands selected";
  }
...
function importTargets(){
  ...
  if (splitEntries.length > 1){
    messageMultiple = "Only <b>one ligand can be imported</b>. Please provide a single uniprot name to be imported.";
  ...
  var msgType = "info";
  if (parsed > 0){
    message = "<b>Successfully</b> imported "+parsed+" ligands.<br>";
    if (notFound.length > 0){
      message += "<br>The following name(s) could <i>not</i> be matched:<br>&#8226;&nbsp;&nbsp;" + notFound.join("<br>&#8226;&nbsp;&nbsp;");
    }
  } else {
    message = "The ligand selection import was <b>not successful</b>. Please make sure you are using the IUPAC ligand names.";
    msgType = "warning";
  }
...
function exportTargets(){
  ...
  if (entries.length > 0){
    ...
    showAlert("The selected ligand has been copied to your clipboard.", "info");
  } else {
    showAlert("There are currently no selected ligands.", "warning");
  }
}
...
function submitSelection(url, minimum = 1) {
  if (selectedTargets.size >= minimum) {
    ...
    .fail(
      ...
      showAlert("Please select a ligand.", "warning");
    }
}
