...
var keys = ['rec_sn', 'rec_gn', 'rec_aa', 'pdb_id', 'int_ty', 'sig_sn', 'sig_gn', 'sig_aa'];
...
var data_t_rec = data_t.filter((thing, index, self) =>
    index === self.findIndex((t) => (
        t.rec_gn === thing.rec_gn
    ))
)
// data that has unique sigprot__generic_name entries
var data_t_sig = data_t.filter((thing, index, self) =>
    index === self.findIndex((t) => (
        t.sig_gn === thing.sig_gn
    ))
)
// interaction types
var int_ty = d3.map(data_t, (d: any) => d.int_ty).keys();
...
var svg = d3.select('body').select('div#content')
    .append('div')
...
var xScale = d3.scaleBand()
    .domain(d3.map(data_t, (d: any) => d.rec_gn).keys().sort(d3.ascending))
    .range([0, w])
    // .round(true)
    .padding(1);
var yScale = d3.scaleBand()
    .domain(d3.map(data_t, (d: any) => d.sig_gn).keys().sort(d3.descending))
    .range([h, 0])
    // .round(true)
    .padding(1);
...
var yAxis = d3.axisRight(yScale)
    .tickSize(0)
    .tickPadding(8);
var xAxisGrid = d3.axisTop(xScale)
...
function infoBoxUpdate() {
    ...
    var info_box = d3.select('g#infobox')
        .selectAll('text')
        .data(info_data);
...
svg.append('g')
    ...
    .enter()
    .append('text')
    .attr('x', function (d: any) { return xScale(d.rec_gn) })
    .attr('text-anchor', 'middle')
    .attr('dy', 75)
    .text(function (d: any) { return d.rec_aa; })
...
svg.append('g')
    ...
    .enter()
    .append('text')
    .attr('y', function (d: any) { return yScale(d.sig_gn) })
    .attr('text-anchor', 'middle')
    .attr('dy', 5)
    .text(function (d: any) { return d.sig_aa; });
...
function tabulate(data, columns) {
    var table = d3.select('div#content').append('table').attr('width', 2 * w)
