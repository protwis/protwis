...
var $jscomp = $jscomp || {};
$jscomp.scope = {};
$jscomp.findInternal = function(c, h, k) {
	c instanceof String && (c = String(c));
	for (var t = c.length, n = 0; n < t; n++) {
		var z = c[n];
		if (h.call(k, z, n, c)) return {
			i: n,
			v: z
		}
	}
...
$jscomp.defineProperty = $jscomp.ASSUME_ES5 || "function" == typeof Object.defineProperties ? Object.defineProperty (c, h, k) {
...
$jscomp.polyfill = function(c, h, k, t) {
	h && ($jscomp.ISOLATE_POLYFILLS ? $jscomp.polyfillIsolated(c, h, k, t) : $jscomp.polyfillUnisolated(c, h, k, t))
};
$jscomp.polyfillUnisolated = function(c, h, k, t) {
	k = $jscomp.global;
	c = c.split(".");
	...
	t = k[c];
	h = h(t);
	h != t && null != h && $jscomp.defineProperty(k, c, {
		configurable: !0,
		writable: !0,
		value: h
	})
...
$jscomp.polyfillIsolated = function(c, h, k, t) {
	...
	null != h && (c ? $jscomp.defineProperty($jscomp.polyfills, n, {
		...
		$jscomp.propertyToPolyfillSymbol[n], $jscomp.defineProperty(t, n, {
			configurable: !0,
			writable: !0,
			value: h
		})))
...
$jscomp.polyfill("Array.prototype.find", function(c) {
	return c ? c (h, k) {
		return $jscomp.findInternal(this, h, k).v
	}
}, "es6", "es3");
(function(c) {
	...
	c.extend(u.prototype, {
		...
		_fnConstruct(a) {
			...
			if ("function" != typeof this.s.dt.oInstance.fnVersionCheck || !0 !== this.s.dt.oInstance.fnVersionCheck("1.8.0")) alert("FixedColumns " + u.VERSION + " required DataTables 1.8.0 or later. Please upgrade your DataTables installation");
				...
				}), c(b.dom.grid.left.head).on("mouseover.DTFC touchstart.DTFC", function() {
					d = "main"
				}));
				0 < b.s.iRightColumns && (c(b.dom.grid.right.liner).on("mouseover.DTFC touchstart.DTFC",
					...
					d = "right";
					b.dom.scroller.scrollLeft -= "wheel" === g.type ? -g.originalEvent.deltaX : g.originalEvent.wheelDeltaX
				}), c(b.dom.grid.right.head).on("mouseover.DTFC touchstart.DTFC", function() {
					d = "main"
				}));
				c(h).on("resize.DTFC", function() {
				...
				m.on("draw.dt.DTFC", function() {
					...
					b._fnColCalc();
					b._fnGridLayout(b)
				}).on("column-visibility.dt.DTFC", function(g, q, p, r, w) {
					if (w === t || w) b._fnColCalc(), b._fnGridLayout(b), b._fnDraw(!0)
				}).on("select.dt.DTFC deselect.dt.DTFC", function(g, q, p, r) {
					"dt" === g.namespace && b._fnDraw(!1)
				}).on("position.dts.dt.DTFC", function(g, q) {
			...
			c.each(this.s.dt.aoColumns, function(f, e) {
				...
				if (e.filter(":visible").length) {
					...
					if (0 === a.s.aiOuterWidths.length) {
						var m = c(a.s.dt.nTable).css("border-left-width");
		...
		_fnCloneLeft(a) {
			if (!(0 >= this.s.iLeftColumns)) {
				var b, d = [];
				for (b = 0; b < this.s.iLeftColumns; b++) this.s.dt.aoColumns[b].bVisible && d.push(b);
		...
		_fnCopyLayout(a, b, d) {
			for (var f = [], e = [], l = [], m = 0, g = a.length; m < g; m++) {
				...
				for (var p = 0, r = this.s.iTableColumns; p <
					r; p++)
					if (-1 !== c.inArray(p, b)) {
						var w = c.inArray(a[m][p].cell, l); - 1 === w ? (w = c(a[m][p].cell).clone(d, !1)[0], e.push(w), l.push(a[m][p].cell), q.push({
							cell: w,
							unique: a[m][p].unique
						})) : q.push({
							cell: e[w],
		...
		_fnClone(a, b, d, f) {
			...
			if (f) {
				...
				q = this._fnCopyLayout(g.aoHeader, d, !1);
				var w = [];
				g.oApi._fnDetectHeader(w, c(">thead", a.header)[0]);
				r = 0;
				for (l = q.length; r < l; r++) {
					var x = 0;
					for (p = q[r].length; x < p; x++) w[r][x].cell.className = q[r][x].cell.className, c("span.DataTables_sort_icon", w[r][x].cell).each(function() {
						this.className = c("span.DataTables_sort_icon", q[r][x].cell)[0].className
					})
				}
			}
			...
			if (0 < g.aiDisplay.length) {
				...
				for (m = 0; m < d.length; m++) {
					var A = d[m];
					var v = c(g.aoColumns[A].nTh).clone(!0)[0];
					v.innerHTML = "";
					p = v.style;
					p.paddingTop = "0";
					p.paddingBottom = "0";
			...
			} else c(">tbody>tr", e.dom.body).each(function(y) {
				v = this.cloneNode(!0);
			...
			if (null !== g.nTFoot) {
				if (f) {
					...
					b.empty();
					r = 0;
					for (l = q.length; r < l; r++) b[0].appendChild(q[r].nTr);
					g.oApi._fnDrawHead(g, q, !0)
				} else
					for (q =
		...
		_fnGetTrNodes(a) {
			for (var b = [], d = 0, f = a.childNodes.length; d < f; d++) "TR" == a.childNodes[d].nodeName.toUpperCase() && b.push(a.childNodes[d]);
			return b
		},
		_fnEqualiseHeights(a, b, d) {
	...
	u.version = "3.3.2";
	n.Api.register("fixedColumns()", function() {
		return this
	});
	n.Api.register("fixedColumns().update()", function() {
		return this.iterator("table", function(a) {
			a._oFixedColumns && a._oFixedColumns.fnUpdate()
		})
	});
	n.Api.register("fixedColumns().relayout()", function() {
		return this.iterator("table", function(a) {
			a._oFixedColumns && a._oFixedColumns.fnRedrawLayout()
		})
	});
	n.Api.register("rows().recalcHeight()", function() {
		return this.iterator("row", function(a, b) {
			a._oFixedColumns && a._oFixedColumns.fnRecalculateHeight(this.row(b).node())
		})
	});
	n.Api.register("fixedColumns().rowIndex()", function(a) {
		a = c(a);
	...
	n.Api.registerPlural("cells().fixedNodes()", "cell().fixedNode()", function() {
		return this.iterator("cell", function(a, b, d) {
			return a._oFixedColumns ? a._oFixedColumns.fnToFixedNode(b, d) : this.cell(b, d).node()
		}, 1)
	});
	c(k).on("init.dt.fixedColumns", function(a, b) {
		if ("dt" === a.namespace) {
			a = b.oInit.fixedColumns;
			var d = n.defaults.fixedColumns;
			if (a || d) d = c.extend({}, a, d), !1 !== a && new u(b, d)
		}
	});
