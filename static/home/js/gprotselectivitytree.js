...
d3.text("GPCRdbtree.txt", function(text) {
...
d3.csv("GPCRdbcsv.csv")
    .row(function(d) { return {name: d.name, red: d.red, black: d.black, green: d.green, blue: d.blue}; })
...
for (i=0; i<myData.length; i++){
    ...
    var leafwithname = vis.selectAll('g.X'+myData[i].name)
        .append("circle")
        .attr("r", 2)
        .style("fill", "red")
  ...
  //var firstlayer = vis.selectAll('g.leaf.node')
    //.append("circle")
    //.attr("r", 2)
    //.style("fill", "black")
    //.attr("transform", "translate(" + (40) + ",0)");
   ...
   //var morenodes=  vis.selectAll('g.leaf.node')
   //.append("circle")
    //.attr("r", 2)
    //.style("fill", "red")
    //.attr("transform", "translate(" + (45) + ",0)");
  ...
  var label = vis.selectAll("text")
    ...
    node.on("click", function(d){
      var subtree = [];
      var subtreeArray = selectSubtree(d, subtree);
      //display in table
...
return subtree;
//return subtree.slice(0, (subtree.length*0.5));
}


function makeUL(descendents){
   
var textarea = document.getElementById('GproteinSubtreeTxtArea');
