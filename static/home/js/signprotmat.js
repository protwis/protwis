...
var data_t_rec = data_t.filter((thing, index, self) => index === self.findIndex((t) => (t.rec_gn === thing.rec_gn)));
// data that has unique sigprot__generic_name entries
var data_t_sig = data_t.filter((thing, index, self) => index === self.findIndex((t) => (t.sig_gn === thing.sig_gn)));
// interaction types
var int_ty = d3.map(data_t, (d) => d.int_ty).keys();
...
var xScale = d3.scaleBand()
    .domain(d3.map(data_t, (d) => d.rec_gn).keys().sort(d3.ascending))
    .range([0, w])
    .padding(1);
var yScale = d3.scaleBand()
    .domain(d3.map(data_t, (d) => d.sig_gn).keys().sort(d3.descending))
    .range([h, 0])
    .padding(1);
