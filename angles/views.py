...
def get_angles(request):
    ...
    try:
        ...
        if len(pdbs)==1 and len(pdbs2)==0:
            ...
            data['data'] = []
            for q in query:
                if q.residue.generic_number != None:
                    data['data'].append([q.residue.generic_number.label,q.residue.sequence_number, q.a_angle, q.b_angle, q.outer_angle, q.hse, q.sasa, q.rsa, q.phi, q.psi, q.theta, q.tau, q.core_distance, q.ss_dssp, q.ss_stride ])
                else:
