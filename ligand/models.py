    ...
    def load_by_name(self, name):
        ...
        try:  # now that we have inchikey, try and see if it exists in DB
            ...
            self.properities.ligand_type, created = LigandType.objects.get_or_create(
                slug='sm', defaults={'name': 'Small molecule'})
...
class AnalyzedAssay(models.Model):
    ...
    efficacy_sign = models.CharField(max_length=3, null=True)
    efficacy_unit = models.CharField(max_length=60, null=True)
    potency = models.FloatField(max_length=60, null=True)
    transduction_coef = models.FloatField(max_length=60, null=True)
    relative_transduction_coef = models.FloatField(max_length=60, null=True)
    delta_relative_transduction_coef = models.FloatField(max_length=60, null=True)
