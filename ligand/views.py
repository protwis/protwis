    ...
    def merge_selectivity_data(self, ligands):
        ...
        for ligand in ligands:
            ...
            B_selective = self.get_count_selectivity(
                "f_selective", "F", protein)
            F_selective = self.get_count_selectivity(
                "b_selective", "B", protein)
...
def TargetSelecitivity(ligand):
    ...
    receptor_list_f = set([item for item in LigandReceptorStatistics.objects.filter(ligand__name=ligand).filter(
        type="F").values_list('protein__entry_name', flat=True).prefetch_related("protein")])
    receptor_list_b = set([item for item in LigandReceptorStatistics.objects.filter(ligand__name=ligand).filter(
        type="B").values_list('protein__entry_name', flat=True).prefetch_related("protein")])
...
class LigandInformationView(TemplateView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context.update({'selectivity': selectivity})
        return context
    ...
    def process_selectivity(self, data):
        ...
        for i in data:
            ...
            if i.type.lower() == "f":
                ...
                type = "Binding"
            selectivity.append({"type": type, "protein": i.protein.entry_name,"potency": i.potency})
        return selectivity
