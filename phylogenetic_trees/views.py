...
class Tree:
	...
	def Prepare_file(self, request):
		...
		self.famdict = {}
		for n in families:
			self.famdict[Tree.trans_0_2_A(n.slug)]=n.name
		...
		self.Additional_info['crystal']={'proteins':[],'colours':['#6dcde1','#6dcde1'],'include':'True'}
		...
		Tree.treeDo('/tmp/%s/outtree' %dirname,self.branches,self.family,self.famdict)
		phylogeny_input = open('/tmp/%s/out.xml' %dirname,'r').read().replace('\n','')
		shutil.rmtree('/tmp/%s' %dirname)
		return phylogeny_input, self.branches, self.ttype, self.total, str(Tree.legend), Tree.box, Tree.defaultColours, self.Additional_info

#	def modify_tree(self,arg):
#		if arg in self.Additional_info:
#            if self.Additional_info[arg]['include']==True:        
#                self.Additional_info[arg]['include']==False
#            else:
#                self.Additional_info[arg]['include']==True


def render_tree(request):
	Treeclass=Tree()
	phylogeny_input, branches, ttype, total, legend, box, defaultColours, Additional_info=Treeclass.Prepare_file(request)
	request.session['Tree']=Treeclass
	return render(request, 'phylogenetic_trees/alignment.html', {'phylo': phylogeny_input, 'branch':branches, 'ttype': ttype, 'count':total, 'leg':legend, 'b':box, 'default':defaultColours, 'add':Additional_info })
